...
- FastAPI resume tokens are opaque session handles. By default they restore the prior browser role from the token; consumers that need identity-aware resume checks must provide `on_resume` validation when constructing `TermHub`.
...
This document describes the intended public contract. It does not mean every backend edge case is perfectly identical today. In particular, verify auth and lease-validation behavior against current tests before treating the two backends as interchangeable for security-sensitive flows.
