...
| `client` | client `hijack`, `control_ws` | `client` | **partial** — the control-channel socket: which of terminal bytes and control frames a payload becomes, the name bytes get back depending on which end is listening, the refusal of a bare JSON object (the one value that reads as a frame and is not one), the refusal of a binary payload, and a truncated frame refused rather than silently dropped. and the hijack client's two guards: an identifier is one safe path segment or it is refused — a whitelist, since escaping is where route forging lives — and nothing whose key looks like a secret is ever written to a log, with long strings and long lists cut so a failure costs a bounded amount. The hijack client's requests themselves outstanding |
