...
| `mcp` | client `ai`/MCP (28 tools) | `mcp` | **partial** — the role ladder and the per-tool policy table, `session_create`'s argument checks (connector allowlist, port range, vetted URL schemes, and the internal-host guard applied to the host *inside* a URL as well as beside it), what a model is shown (a snapshot's three shapes, two of which strip ANSI), and the authorization chokepoint every call passes through: judged on the best of several roles, a principal resolved from the request before the server's default, a lookup that fails treated as having said nothing, and a tool with no policy refused before a principal is even resolved. Also the input guards every tool funnels caller-supplied input through — the structural ReDoS denylist with its length cap, and the `invalid_pattern`/`invalid_id` refusal contracts — and all twenty-nine tool bodies (with `pycompat`'s `pyStr`, so a screen rendered before matching reads as the reference reads it) — the ten hijack and control tools, the twelve session, fan-out and annotation tools with their clamps and their pattern re-check, and the seven that reach a graphical console — each validating before it reaches and folding whatever the client says into one answer shape |
