...
| `cli` | server CLI (`uterm`) | `cli` | **partial** — `uterm share`'s decisions: where the bearer token comes from (flag, named file, default file, nothing), what the session is called, where the WebSocket is once the server's answer is resolved against the server it came from, what is printed, and the bridge that carries bytes both ways until either side closes. The remaining subcommands outstanding |
