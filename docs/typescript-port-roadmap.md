...
| `connectors` | server `connectors/*` | `connectors` | **partial** — the registry, the connector interface, the reference shell connector and the telnet connector with its closed settings and post-connect address guard; the ssh/websocket connectors outstanding |
