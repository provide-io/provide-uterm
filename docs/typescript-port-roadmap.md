...
| `cloudflare` | `provide-uterm-cloudflare` (53 files, ~8.3k lines) | — | **partial** — the Worker configuration with its startup refusals, the KV session registry, and the Durable Object's schema, session metadata, invite state and lease state; the event log, snapshots, webhooks, resume tokens, Worker entry, transport, auth and routes outstanding |
