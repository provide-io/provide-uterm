...
| `render` | `render` | `render` | **done** — SGR row rendering, the colour palettes with their quantisers, the structured segment stream, and image-to-terminal-art rendering. Decoding an image is the caller's: the reference takes it from PIL, and this takes pixels from whatever the caller has |
...
| `transports` | client `transports/*` | `transports` | **partial** — the transport interface, telnet RFC 854 framing and negotiation, the reconnect budget/backoff and the reconnecting session's full ordering — the dead socket closed before the backoff, the new session live before the reconnect hook, and the two exhaustion messages kept apart — the WebSocket client, the chaos wrapper, the full RFC 854 telnet client, the SSH session stream adapters, and the SSH server's admission rules — the loopback-bind test, the refusal to start unauthenticated on a routable address, the host-key permission check and the per-address connection cap — and the telnet server itself: the opening preamble that claims echo and full duplex, turns the client's own line editing off and asks for the window size, then a real listener that greets before it hands over, closes the connection however the handler ended, keeps clients apart and reports a bind failure rather than hanging on it |
...
| `serverauth` | server `auth*`, `webhook*`, `api_keys`, `dev_idp` | `serverauth` | **partial** — webhook signing, the RBAC allow-list, the API-key store, the tunnel token hash, and now the whole `jwt`-mode identity path: the HS256 verifier written out rather than taken from a library (because the *refusals* are the contract — expired, signed by somebody else, minted for another audience, carrying an algorithm the deployment did not allow, and each of the ways a segment can be malformed, in the order PyJWT reaches them), the bearer-header grammar, roles and scopes read off a claim set through the allow-list, and the `dev_token` stub IdP — which mints an HS256 secret and a real JWT and then rewrites the config to `jwt`, so a port cannot pass the live matrix by recognising its own token instead of verifying it. The `header`, `api_key` and `webhook` modes outstanding |
| `serverconfig` | server `config*`, `profiles` | `serverconfig` | **partial** — the outbound-URL guard, mount-path normalisation, every cross-field validator, TOML loading with its structural pass and relative-path resolution, the whole default configuration, the security-posture report, the security response headers, the saved connection profiles with their mutable-field allow-list, every field's own type, choice set and refusal of names nobody defined, and the session definitions with their identifier pattern, connector-type registry and connector-config fold — **complete** apart from date-time parsing (see the divergences below) |
...
| `server` | server `routes/*`, `app/*`, `runtime`, `bridge/routes/rest`, `app/hub_authz` | `server` | **partial** — binding the shared contract to a backend's handlers, with its capability and role-guard validation and a framework-neutral dispatcher; and a **runnable server**: the health, liveness and readiness endpoints with their two 503s and CPython's own rounding of the uptime, the RBAC decisions (`ROLE_CAPABILITIES`, the scope narrowing, the session-scoped admin grant that is deliberately not a global one, `can_read_session` and `can_mutate_session` — which is deliberately *not* symmetric with the read, because visibility says who may watch and never who may change), the session store with its list narrowing, ordering and paging, the session status object field for field, the bootstrap that stands the stub IdP up and refuses `dev_token` on anything but loopback, and the whole request path — route before authenticate, authenticate before existence, one refusal for both ways of failing to authenticate. Held to `serverhttp_golden`, which is what the reference FastAPI server answered on a real socket. **The hub is now composed** (`session-hub.ts`: all nine services wired to each other, plus the `append_event` / `send_worker` / `set_input_mode` / `update_last_snapshot` / `prune_if_idle` / `force_release_hijack` methods the reference keeps on the composed hub) and **a started session attaches to it as a real worker** (`worker-attach.ts`: the ported `WorkerLink` on one side, a `ControlFrameDecoder` on the other, joined in-process where the reference loops back over its own WebSocket) — so `connected` now means what it means there. On top of that: `GET /api/sessions/{id}/snapshot`, `POST /api/sessions/{id}/mode`, and the whole REST lease family — `acquire`, `heartbeat`, `snapshot`, `send`, `step`, `release` — behind the `hub_authz` gate, with **both** refusal envelopes kept apart (`detail` from the gate, `error` from the lease). Held to `serverhijack_golden`, a 29-probe *sequence* recorded in order off the running reference with its worker attached. The write half of the session family (`create`, `update`, `delete`, `bulk_delete`), the remaining lifecycle and data sub-routes, the hijack events poll, the REST rate limiter and the WebSocket surfaces are outstanding |
| `client` | client `hijack`, `control_ws` | `client` | **done** — the control-channel socket (which of terminal bytes and control frames a payload becomes, the name bytes get back depending on which end is listening, the refusal of a bare JSON object, of a binary payload, and of a truncated frame) and the hijack client: its two guards — an identifier is one safe path segment or it is refused, and nothing whose key looks like a secret is ever written to a log — and its whole request surface, every method, path and body driven against the reference with the transport replaced by a recorder |
| `gateway` | server `gateway` | `gateway` | **done** — the SSH server's security policy (loopback bind test, host-key permissions, per-IP limits, permissive-auth refusal), the host-key store (including an OpenSSH ed25519 encoder written from the format, because Node emits only PKCS#8), the SSH server, the WebSocket server, the telnet gateway's IAC negotiation, and the telnet listener itself with its refusal to bind anywhere but loopback without an explicit opt-in. All three servers are stood up on an ephemeral port and connected to for real |
| `tunnel` | `tunnel`, `tunnel_invites`, `pam_tunnel` | `tunnel` | **partial** — the one-time invite bootstrap (a store keyed by digests, an invite spent whether or not redeeming it worked, a life of five minutes or the tunnel's own, and the session checked against the caller's) and the BLAKE2b-256 token hashing under it, plus the wire format — two header bytes and a payload, with control messages on channel zero as compact ASCII-escaped JSON that must name a type. The tunnel transport itself and `pam_tunnel` outstanding |
| `tunnelclient` | share/inspect client | `tunnelclient` | **partial** — HTTP request interception: the denylist that stops an operator's rewrite touching hop-by-hop, framing or identity headers (matched without regard to case), the decision parser with its fall back to forwarding and its refusal of a body it would have to guess at, and the gate — a paused request released the configured way rather than left hanging, the first decision standing against a second, and everything released at once on shutdown. The proxy's reporting: a body carried only when it is worth reading — something binary reported by size alone, since a megabyte of base64 nobody can read is a megabyte spent, and something too large marked truncated — with the content type read as the type alone, and the one-line log with its direction, its round-to-even duration and its mark on a failure at the far end. and the tunnel transport itself: the frames a worker puts on the wire — an `open` naming the terminal's size, data on a channel, a resize, an end-of-file — the token carried as a bearer header rather than in the address, a refusal to send anything before there is a connection, and a reconnect that waits before each attempt on a schedule that saturates |
| `vnc` | `vnc` | `vnc` | **done** — the RFB client-input filter, which is the boundary that stops a viewer typing into a session it may only watch, and the human relay with its four orderings: the update driver waits for the client's first request, every write upstream is serialised, teardown stops the driver first, and a shutdown race is reported rather than raised. A live session supplies an asynchronous input filter (and writes through the relay's own `write`); the bundled filter is synchronous and suits a buffered source |
| `shell` | `shell` | `shell` | **done** — the terminal output formatting (messages, labelled values, fixed-width tables) and the command dispatcher: a command is the first word lowercased, everything after it is one argument, an empty line and a bare interrupt are a prompt rather than an error, and a line naming nothing says so and says where to look. The commands that reach outside are supplied by the host rather than built in, so a runtime without one refuses it by name instead of pretending. Image rendering is re-exported from `render`. Its Python evaluation sandbox is **not planned** and is the reason `py` is a host-supplied command here: restricting Python builtins has no counterpart on this runtime, and inventing an evaluation sandbox rather than porting one would be a new security surface wearing a port's clothes |
| `manager` | External Management Tier | `manager` | **partial** — the token boundary: the operator/worker privilege split, the per-agent HMAC derivation that binds a worker's token to the agent id in its path (so one compromised worker cannot report as, or register over, any other), the fully-anchored self-report routes, the fleet-token backward-compatibility setting, token extraction from a bearer header, an api-token header or a WebSocket query, and the two refusal shapes. and the timeseries reader: where the current run starts (a sharp fall in turns, by a share *and* a floor, or the agents going from some to none) and what a malformed line does, which is nothing — a reader that failed on one bad line would lose every good one behind it. and the two supervision decisions that bound what a compromised worker can do: a worker never inherits the operator token — where a fleet secret is configured its environment gets one derived from that secret and bound to its own agent id, and the raw secret is stripped whether one is configured or not — and every agent gets a name nobody else holds, taken past the highest already known rather than into the first gap. The routes and the spawning itself outstanding |
| `mcp` | client `ai`/MCP (28 tools) | `mcp` | **partial** — the role ladder and the per-tool policy table, `session_create`'s argument checks (connector allowlist, port range, vetted URL schemes, and the internal-host guard applied to the host *inside* a URL as well as beside it), what a model is shown (a snapshot's three shapes, two of which strip ANSI), and the authorization chokepoint every call passes through: judged on the best of several roles, a principal resolved from the request before the server's default, a lookup that fails treated as having said nothing, and a tool with no policy refused before a principal is even resolved. Also the input guards every tool funnels caller-supplied input through — the structural ReDoS denylist with its length cap, and the `invalid_pattern`/`invalid_id` refusal contracts — and all twenty-eight tool bodies (with `pycompat`'s `pyStr`, so a screen rendered before matching reads as the reference reads it) — the ten hijack and control tools, the eleven session, fan-out and annotation tools with their clamps and their pattern re-check, and the seven that reach a graphical console — each validating before it reaches and folding whatever the client says into one answer shape. The earlier "twenty-nine" was an off-by-one in this row's own sub-count, not a TypeScript-only extra tool: `callHijackTool` and `callSessionTool` (`src/mcp/hijack-tools.ts`, `src/mcp/session-tools.ts`) dispatch exactly the reference's 28 tool names, no more and no fewer |
| `audit` | server `audit_chain` | — | **done** — the tamper-evident chain: the canonical form a record's hash is taken over, and a verifier that catches a record altered, removed, inserted or reordered, and — given the head somebody remembers — the end cut off, which nothing inside the log can show |
| `cli` | server CLI (`uterm`) | `cli` | **partial** — `uterm share`'s decisions: where the bearer token comes from (flag, named file, default file, nothing), what the session is called, where the WebSocket is once the server's answer is resolved against the server it came from, what is printed, and the bridge that carries bytes both ways until either side closes. and `uterm audit verify`, which is the chain above with a usage rule: a head is a pair, so naming one half of it without the other is an error rather than half a check. and `uterm watch`'s argument, which takes a link or an identifier: the tunnel is read out of the path and never out of the query, so a link whose path names nothing and whose query names a tunnel is not followed to it. and `uterm inspect`'s decisions: where the WebSocket is, a session named after the port being inspected when nobody named it, the tunnel named by whichever field the server used, and what is printed — including that interception is on, since a proxy that pauses requests looks exactly like one that has hung. `uterm tunnel` outstanding |
| `cloudflare` | `provide-uterm-cloudflare` (53 files, ~8.3k lines) | — | **partial** — the Worker configuration with its startup refusals, the KV session registry, and the whole Durable Object state store (schema, session metadata, invite state, lease state, snapshots, event log, recording view, webhooks and resume tokens); the session runtime's flow controller, its WebSocket attachment reading, webhook secret encryption, the polling server-sent-events endpoint, the whole JWT auth path (token splitting, JWKS key choice and its caching endpoint, standard-claim validation, RS256 signature verification, role derivation, principal derivation and token extraction), the session runtime's own auth (share-cookie roles with their expiry and address binding, and the owner elevation), its socket registry, and how it frames what it sends; the Worker entry, the rest of the session runtime, transport, auth and routes outstanding |
...
| `cloudflare` | a request body that is not valid JSON returns nothing, where the reference raises | `request_json` returns an empty object for every other bad input — wrong content type, oversized, empty, not an object — but lets a `JSONDecodeError` escape. Whether that surfaces as a 500 depends on a handler above it this port has not traced; the inconsistency within the function is certain. **Worth checking upstream** |
| `gateway` | a subnegotiation whose terminating `IAC SE` is split across two reads still completes, where the reference loses it | Inside a subnegotiation the reference appends a dangling `IAC` to the payload as a literal instead of holding it, so the sequence never ends: the terminal type is never learnt and the handshake never completes. A two-byte terminator landing on a segment boundary is ordinary, so this is reachable from a real client. **Worth fixing in the reference** |
| `gateway` | an option nobody asked about gets silence rather than a refusal | Carried over. RFC 854 asks for `DONT`/`WONT`, and a client that waits for one waits forever; whether any client in practice does is not something this port can answer, so it does what the reference does |
| `tunnelclient` | a request stops being counted as pending the moment it is decided, where the reference counts it until the coroutine awaiting it resumes | A window this runtime has no equivalent of. The count answers "how many are still undecided", which is what it is read as |
| `transports` | a missing file, a denied permission and an interrupted call are not retried, where the reference retries all three | All three are `OSError` in Python, so the reference spends the whole budget on them. None is a transport that might come back: the next attempt fails identically, and retrying only delays the report |
| `transports` | after giving up, `session` names nothing, where the reference keeps pointing at the session it just closed | A later call there fails against the dead session and rebuilds; this one connects afresh. Same end state, and no handle to a closed socket in between |
| `tunnel` | an invite whose stored expiry is `NaN` or infinite is refused, where the reference takes it | `now > nan` is false in both languages, so such an invite never expires there. Neither value can arrive through JSON, and refusing is the safe direction |
| `tunnel` | an invite whose stored token is `null` is refused, where the reference hands back the token `"None"` | `str(None)`. It authenticates nobody — it cannot verify against the tunnel's hash — but answering "here is your token" for a store written wrong is worse than refusing |
| `mcp` | which of two equally-ranked roles `primaryRole` names is the port's own answer, not a matched one | The reference holds roles in a `frozenset`, so its `max` over two unrecognised roles — which rank alike — depends on hash order. The port keeps the first given. Display only: no decision consults it |
| `mcp` | a snapshot that is not a mapping is passed through, where the reference raises | `_clean_snapshot` reads `.get` off whatever the server put under `snapshot`, so a bare string raises inside the tool and the caller sees a tool error instead of the answer. Only a mapping is cleaned here and anything else is left as it arrived: a malformed answer says more than a stack trace, and nothing downstream reads it as a screen |
| `mcp` | what counts as a *valid* regex is the host engine's, so `(a)\2` — an invalid group reference to the reference — compiles here as a legacy escape | The two engines never agreed on validity or on how a refusal reads, and neither bounds matching time. What both ports do share is enforced exactly: the length cap, the structural denylist, and the refusal contract around whatever the engine says. The pattern matches a control character rather than a group, and a filter that matches nothing shows nothing |
| `mcp` | `100.64.0.1` (RFC 6598 carrier-grade NAT) is **not** treated as internal | Neither runtime classifies it as private, so an LLM may point a session into a CGNAT range. Faithful to the reference and recorded here because it is the kind of gap that reads as an oversight rather than a decision |
| `graphical` | an endpoint whose bracket is never closed is refused with a code, where the reference raises | `parse_rfb_endpoint` guards only its port lookup, so `urlparse` raises a bare `ValueError` on `[2001:db8::1:5900` and an operator's typo becomes a 500 where it earned a 400. Same in `parse_litevirt_endpoint`. **Worth fixing in the reference** |
| `graphical` | a target seeded into a *closed* registry is still accepted, as in the reference | `add_static` is the one operation that consults neither the scope nor the closed flag. Carried over: seeding is what a deployment does to itself at startup rather than something a caller asks for, so the port records the asymmetry instead of tightening it |
| `pty` | a PAM notify line that is valid JSON but not an *object* returns nothing, where the reference raises | `_parse_event` documents itself as returning `None` on any error, but calls `.get` on a list, a string or a null and raises `AttributeError`. The exception leaves the connection handler, so one such line from any sender past the socket's 0600/peer-uid gate ends that connection and logs a traceback. The port does what the docstring says. **Worth fixing in the reference** |
| `serverconfig` | `created_at` on a session definition is checked for shape, not parsed | The reference uses Pydantic's own date-time grammar; reproducing its error taxonomy is a unit of its own, so the port checks the field is a string or a number and leaves the instant to whoever reads it |
| `serverconfig` | a name nobody defined in a `[[sessions]]` entry becomes a connector setting rather than a startup failure | The reference's own before-validator folds unrecognised keys into `connector_config`, which defeats `extra="forbid"` for that one section: a mistyped `recording_enabled` configures nothing and recording stays off. Carried over rather than corrected — a connector's options are open-ended and have to reach it somehow |
| `server` | a 405 names only the verbs this port binds, where the reference names the whole session family | The reference binds all nineteen session capabilities, so `PUT /api/sessions/{id}` answers `Allow: DELETE, GET, PATCH`; this port binds the read half and answers `Allow: GET`. Advertising operations it cannot serve would send a client to a verb that 404s, so the header shrinks with the binding. Recorded as an assertion in `server/app.test.ts` against the reference's own recorded header |
| `server` | an invalid `?limit=` or `?q=`, or a malformed `worker_id`/`hijack_id` in a lease path, is a 422 carrying one `detail` string, where the reference carries its framework's validation envelope | Same status, different body: reproducing FastAPI's per-field error list (`type`, `loc`, `msg`, `input`, `ctx`) is a unit of its own and no scenario reads it. The refusal itself is exact — the same bounds, refused rather than clamped |
| `server` | the REST lease routes are not rate-limited, where the reference answers `429 {"error": "rate_limited"}` past a per-client-address budget | The hub composition (`session-hub.ts`) leaves the rate limiter and the approval store out, so there is nothing to consult. A gap rather than a decision, and stated in that file's own header as well as here — the limiter is ported (`ratelimit/`) and only the wiring is missing |
| `server` | `/api/security-posture` is not served | The reference's one authenticated endpoint outside the shared contract. Its body would diverge anyway until the declared auth mode is carried past `setup_dev_idp`'s rewrite, which the reference does with a private attribute |
| `serverauth` | the one refusal whose text is CPython's codec message is worded differently | `Invalid header string: 'utf-8' codec can't decode byte 0x9e in position 0: invalid start byte` — PyJWT hands `json.loads` raw bytes, so the message is CPython's UTF-8 error taxonomy. The refusal *kind* (`DecodeError`) matches, which is what anything downstream reads; the text is logged and never served. Asserted as a divergence rather than left as a comment |
| `serverauth` | an asymmetric algorithm is refused even against a real PEM key | Only HS256 is implemented. Against a shared secret — what a `dev_token` deployment has — the reference refuses identically (`InvalidKeyError`, "Could not parse the provided public key."), and this matches it. Against a genuine RS256 deployment the reference would verify and this port fails closed |
...
| `cloudflare/ws-send` | A `null` in a terminal frame's data stringifies to `"null"`, not `"None"` | The same `str(None)` difference; unreachable from the session, which never builds a terminal frame without data |
