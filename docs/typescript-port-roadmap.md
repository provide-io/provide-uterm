...
| `serverconfig` | server `config*`, `profiles` | `serverconfig` | **partial** — the outbound-URL guard, mount-path normalisation and every cross-field validator; TOML loading and profiles outstanding |
...
| `cloudflare` | `provide-uterm-cloudflare` (53 files, ~8.3k lines) | — | **partial** — the Worker configuration with its startup refusals, the KV session registry, and the Durable Object's schema, session metadata, invite state, lease state, snapshots and event log; webhooks, resume tokens, recording entries, Worker entry, transport, auth and routes outstanding |
