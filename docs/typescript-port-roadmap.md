...
| `mcp` | a screen that is `null` matches nothing, where the reference matches the text `None` | `session_subscribe` re-checks the pattern against each event's screen and renders a non-string one with `str()`, so a null screen becomes the four characters `None` and a pattern like `^N` fires on a screen the terminal never showed. A screen that is nothing matches nothing here: the alternative is a filter firing on an artifact of how the absence was written down |
