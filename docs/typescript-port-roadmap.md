...
| `server` | server `routes/*`, `app/*`, `runtime` | `server` | **partial** — binding the shared contract to a backend's handlers, with its capability and role-guard validation and a framework-neutral dispatcher; and a **runnable server**: the health, liveness and readiness endpoints with their two 503s and CPython's own rounding of the uptime, the RBAC decisions (`ROLE_CAPABILITIES`, the scope narrowing, the session-scoped admin grant that is deliberately not a global one, and `can_read_session`), the session store with its list narrowing, ordering and paging, the session status object field for field, the bootstrap that stands the stub IdP up and refuses `dev_token` on anything but loopback, and the whole request path — route before authenticate, authenticate before existence, one refusal for both ways of failing to authenticate. Held to `serverhttp_golden`, which is what the reference FastAPI server answered on a real socket. The write half of the session family (`create`, `update`, `delete`, `bulk_delete`), the lifecycle and data sub-routes, the WebSocket surfaces and the session runtime are outstanding |
...
| `conformance/live` | run the live scenarios against the TS server | **done** — `typescript` is registered in both roles and all sixteen cells (four scenarios × two servers × two clients) pass against the Python reference |
