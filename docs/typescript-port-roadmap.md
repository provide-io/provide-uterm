...
| `hub` | server `bridge/hub` (nine services) | `hub` | **partial** — router, polling, presence, lease manager, worker state model, state store, rate limiter, worker registry and approval store; one outstanding (connections) |
