...
- **B-F10-session (gated on U2):** Refactor the explorer MCP `session_send/snapshot/step/release` tools (`mcp/server.py:195-254`) onto `session.send_expect(...)` where they drive a local session; keep the game-domain tools custom.
