...
pending human approval via Slack/webhook/REST. Teleport does post-hoc audit;
this is real-time interception. ARD: `docs/ard-command-approval-workflows.md`.

### 2. Session Replay with AI Annotation
Replay recordings with AI-generated annotations: "operator escalated privileges
here", "this curl leaked credentials", "deployment started at this point".
Turns compliance recordings into searchable, indexed knowledge. No ARD yet.

### 3. Multi-Session Fan-Out
Broadcast input to N sessions simultaneously. Detect output divergence across
sessions. Per-session replay. Fleet ops without Ansible. Combined with MCP
tools, AI agents can orchestrate rolling deployments across terminal fleets.
ARD: `docs/ard-multi-session-fanout.md`.
