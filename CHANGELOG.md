...
- **Error responses no longer echo internal exception text.** A failed
  graphical attach, fan-out create, agent spawn, hub call or session create
  answered with the exception's own message — socket errors, paths, library
  internals — in the response body. Every port now returns a fixed message and
  logs the detail server-side: Python (`rest_gui`, fan-out routes, manager
  `spawn`/`agent_ops`), Go (GUI operations, hub calls, session create), C# (GUI
  attach and the manager's catch-all 500) and the TS GUI attach.
- **An open redirect in the C# tunnel routes is closed.** C# gains
  `UiPaths.Clean` and `UiPaths.MountPrefix`, and `_clean_path` now collapses
  leading `/ \ \t \r \n` the same way in all four ports.
- **Host-key file writes are hardened against symlink/TOCTOU races** (Python
  `ssh.py`, TS `host-key.ts`), and the TS gateway keeps client-sent telnet
  environment variables in a `Map`, so a client naming one `__proto__` cannot
  pollute the object prototype.
- **Patched dependency floors.** asyncssh `>=2.24.0` in the published packages
  (GHSA-2wxc-x7rj-hg8f, SCP path traversal, HIGH; plus GHSA-rw4j-r22c-9gc3,
  GHSA-qr67-gv47-xwwh, GHSA-g794-3fmp-753h). The Cloudflare image reads its
  wrangler version from `package.json` instead of a stale literal, clearing
  sharp GHSA-rgj7-g3m4-5g8c (HIGH), and bumps undici to 6.28.1. grpc stays on
  1.83.2: 1.84.x is inside CVE-2026-84445 (HIGH) and Dependabot ignores it
  until 1.85 ships.
- **The TS port refuses non-canonical base64url JWT segments,** matching PyJWT
  2.14.

### Added

- **Every port reports who closed a connection.** A transport close is now a
  typed `TransportClose` (initiator, code, reason, detail) in Python, TS, Go
  and C#, exposed as `close_info` on the session and `last_close` on the
  reconnect wrapper. The existing catch sites still work: Go's
  `errors.Is(err, ErrConnectionClosed)` matches, and C#'s
  `TransportClosedException` is an `IOException`. All four ports are held to
  one shared `close_cases` vector set, and the spec gains a `transport_close`
  category.
- **C# `ReconnectingTransport`,** a port of Go's reconnect wrapper (backoff
  policy, injectable sleeper, `RetriesExhaustedException`, `LastClose`), held
  to the same `reconnect_golden.json` corpus as the TS port.

### Fixed

- **The Python server did not start with `ui.app_path = "/"`.** FastAPI
  refuses a router prefix ending in `/`, and past that every link built as
  `{app_path}/session/...` began `//` — a protocol-relative URL pointing
  off-site — in the `/s/<id>` redirect, the quick-connect and tunnel URLs and
  the frontend bootstrap. They now mount and join on `mount_prefix()`, which
  maps the root to an empty prefix, as C#'s `UiPaths.MountPrefix` already did.
- **The C# manager crashed on `/swarm/agents/stop`.** A path with no agent id
  between the prefix and the `/stop` suffix threw `ArgumentOutOfRangeException`
  on the id slice, and the catch-all sent the exception text back. It is now a
  404.
- **The hostile-client CI leg installed the .NET 8 SDK to run a `net10.0`
  project,** and worked only because the runner image preinstalls .NET 10.

### Changed

- **The Python server image runs on `python:3.14-slim-trixie`.** The image is
  one deployment and runs one interpreter; `requires-python` stays `>=3.11` and
  CI keeps testing 3.11–3.14. The Go builder and the VNC lab image move to
  Debian trixie as well.
- **Go 1.27.1** (from 1.26.6), with golangci-lint 2.13.2.
- **ty is diagnostic-clean and now gates CI** (`--error-on-warning`). Most of
  the 75 diagnostics came from ty treating the mutmut `./src` symlink tree as
  first-party.
- **mutmut 3.8.0,** with the perimeter back to `killed==100`. 3.8 also mutates
  ternary conditions and methods of decorated classes, so new survivors were
  killed or allowlisted with reasons, and renumbered allowlist ids re-keyed.
- **The frontend moves to vitest 5;** the TS port stays on vitest 4 because
  Stryker's vitest runner records no per-test coverage under 5.
- **Dependabot sends one grouped PR a week** across every ecosystem, and now
  also updates pre-commit hook revs and scans `docker/vnc-lab`.
- Dependencies upgraded across Python, npm, Go, NuGet (with dotnet-stryker 5),
  GitHub Actions and the pre-commit hooks.
