...
#   ci/typecheck.sh ty             — strict: fail on any ty diagnostic
#
...
#   - ty (Astral's type-checker) gates too: the repository is diagnostic-clean
#     on the locked ty, so any new diagnostic (errors AND warnings, e.g. an
#     unused `ty:ignore`) fails. Every package is checked before exiting so one
...
case "${mode}" in
  ...
  mypy-soft)
    ...
    ;;
  ty)
    # Gating. Empty SOFT_PACKAGES must not trip `set -u` on
    # "${SOFT_PACKAGES[@]}" (bash unbound-variable).
    rc=0
    ...
    for pkg in "${packages[@]}"; do
      echo "::group::ty ${pkg}"
      uv run ty check --error-on-warning "${pkg}" || rc=$?
      echo "::endgroup::"
    done
    if [ "${rc}" -ne 0 ]; then
      echo "::error::ty reported diagnostics (see groups above)"
    fi
    exit "${rc}"
    ;;
  *)
