...
_skip_needs_pam = pytest.mark.skipif(not _PAM_SVC.exists(), reason="requires /etc/pam.d/provide-uterm")
_skip_needs_pam_auth = pytest.mark.skipif(
    not _PAM_SVC.exists() or _IN_DOCKER,
    reason="requires /etc/pam.d/provide-uterm with working unix_chkpwd (skipped in Docker)",
)
...
def test_bad_credentials_raises_pam_error() -> None:
    """Requires /etc/pam.d/provide-uterm and a 'testuser' OS account."""
    session = PamSession()
...
def test_good_credentials_succeed() -> None:
    """Requires /etc/pam.d/provide-uterm and testuser:testpass123."""
    session = PamSession()
