...
```bash
pip install provide-shell
```

## CLI

```bash
python -m provide.shell
```
...
`provide-shell` is the engine behind the `ushell` connector type in
...
On Cloudflare Workers, `provide-terminal-cloudflare` vendors `provide-shell` into the
