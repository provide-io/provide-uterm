...
@pytest.mark.asyncio
async def test_sqlite_migration_preserves_audit_head_table_through_latest_version(tmp_path: Path) -> None:
    db_path = tmp_path / "cp.db"
    plane = SqliteControlPlane(ControlPlaneConfig(database_url=str(db_path)))
    await plane.migrate()
    await plane.close()
...
@pytest.mark.asyncio
async def test_sqlite_migrate_v1_db_forward_to_latest(tmp_path: Path) -> None:
    """A db already at v1 migrates through every later version cleanly."""
    db_path = tmp_path / "cp.db"
    ...
    # Re-migrate via the normal path: v1 is skipped and later versions are applied.
    plane = SqliteControlPlane(ControlPlaneConfig(database_url=str(db_path)))
    await plane.migrate()
