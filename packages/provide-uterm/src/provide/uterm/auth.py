...
#
"""Pluggable SSH-key-based authentication for provide-uterm gateways.

This module defines the boundary between *transport* (which provide-uterm
...
Two reference resolvers ship with provide-uterm:
...
def _parse_authorized_keys_line(line: str) -> _AuthorizedKeyEntry:
    ...
    subject_opt = opts.pop("subject", None)
    subject = subject_opt if isinstance(subject_opt, str) and subject_opt else comment.strip() or f"key:{fp}"
    claims: dict[str, Any] = {}
