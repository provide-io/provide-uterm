...
# stay in sync with the FastAPI backend (provide-uterm SessionRuntimeStatus
...
class SessionStatusItem(SessionStatusResponse):
    ...
    Mirrors ``provide-uterm`` ``SessionRuntimeStatus``/``SessionStatus`` (TS).
...
def _normalize_frame(value: dict[str, Any], *, limits: MessageLimits) -> Frame:
    ...
    if frame_type == "input":
        ...
        # Browser → DO cumulative-bytes ACK for Tier-A backpressure. Coerce to a
        # non-negative int; a non-numeric value normalizes to 0 (never raises).
        raw_bytes = value.get("bytes")
        cast("dict[str, Any]", normalized)["bytes"] = raw_bytes if isinstance(raw_bytes, int) else 0
    elif frame_type in {
        "snapshot_req",
        "error",
        "worker_connected",
...
class RuntimeProtocol(Protocol):
    ...
    async def resolve_principal(self, request: object) -> tuple[Any, Any | None]: ...
