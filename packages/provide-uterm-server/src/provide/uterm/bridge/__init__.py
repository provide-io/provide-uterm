...
- :class:`~provide.uterm.bridge.hub.TermHub` — server-side registry.
  Tracks which workers are connected, manages leases, routes input/output.

- :class:`~provide.uterm.bridge.worker_link.TermBridge` — worker-side WS client.
  Connects the worker to the hub, forwards terminal output, receives control commands.

- :mod:`~provide.uterm.bridge.routes` — FastAPI WebSocket + REST routes.
