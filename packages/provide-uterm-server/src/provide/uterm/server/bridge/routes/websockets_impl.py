...
async def _ws_worker_term(hub: TermHub, websocket: WebSocket, worker_id: str) -> None:
        ...
        if should_broadcast:
            ...
            if was_hijacked:
                ...
                _hijack_state_task.add_done_callback(
                    lambda t: (
                        logger.warning("broadcast_hijack_state_failed worker_id=%s error=%s", worker_id, t.exception())
                        if not t.cancelled() and t.exception() is not None
                        else None
                    )
                )
...
async def _ws_browser_term(hub: TermHub, websocket: WebSocket, worker_id: str) -> None:
    await websocket.accept()
...
def register_ws_routes(hub: TermHub, router: APIRouter) -> None:
    ...
    async def ws_worker_term(websocket: WebSocket, worker_id: Annotated[str, Path(pattern=r"^[\w\-]+$")]) -> None:
        await _ws_worker_term(hub, websocket, worker_id)
    ...
    async def ws_browser_term(
        ...
        await _ws_browser_term(hub, websocket, worker_id)
