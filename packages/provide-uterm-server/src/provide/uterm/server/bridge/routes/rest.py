...
# The seven route bodies below are kept out of the decorated closures and
# delegate to these module-level, undecorated implementations because mutmut
# skips decorated functions entirely (see docs/mutmut-survivors-triage.md
# Wave 9) — without this split, ~400 lines of lease logic generate zero
# mutants.


async def _hijack_acquire(
    hub: TermHub,
    http_request: Request,
    worker_id: str,
    request: HijackAcquireRequest | None,
) -> Any:
    """Acquire a hijack lease; the body of ``POST /worker/{id}/hijack/acquire``."""
...
async def _hijack_heartbeat(
    hub: TermHub,
    worker_id: str,
    hijack_id: str,
    request: HijackHeartbeatRequest | None,
) -> Any:
    """Extend a hijack lease; the body of ``POST /worker/{id}/hijack/{hid}/heartbeat``."""
    if request is None:
...
async def _hijack_snapshot(
    hub: TermHub,
    worker_id: str,
    hijack_id: str,
    wait_ms: int,
) -> Any:
    """Fetch a hijack snapshot; the body of ``GET /worker/{id}/hijack/{hid}/snapshot``."""
    hs = await hub.get_rest_session(worker_id, hijack_id)
    if hs is None:
        return JSONResponse({"error": "Invalid or expired hijack session."}, status_code=404)
...
async def _hijack_events(
    ...
    """Fetch hijack events; the body of ``GET /worker/{id}/hijack/{hid}/events``."""
    hs = await hub.get_rest_session(worker_id, hijack_id)
    if hs is None:
        return JSONResponse({"error": "Invalid or expired hijack session."}, status_code=404)
...
async def _hijack_send(
    hub: TermHub,
    http_request: Request,
    worker_id: str,
    hijack_id: str,
    request: HijackSendRequest,
) -> Any:
    """Send keys to a hijacked worker; the body of ``POST /worker/{id}/hijack/{hid}/send``."""
    _client_id = (http_request.client.host if http_request.client else None) or "unknown"
    if not hub.allow_rest_send_for(_client_id):
        hub.metric("rest_send_rate_limited_total")
        logger.warning("rest_send_rate_limited worker_id=%s hijack_id=%s client=%s", worker_id, hijack_id, _client_id)
        await hub.emit_telemetry(
            "rate_limit.triggered",
            worker_id=worker_id,
...
async def _hijack_step(
    hub: TermHub,
    http_request: Request,
    worker_id: str,
    hijack_id: str,
) -> Any:
    """Step a hijacked worker; the body of ``POST /worker/{id}/hijack/{hid}/step``."""
    _client_id = (http_request.client.host if http_request.client else None) or "unknown"
    if not hub.allow_rest_send_for(_client_id):
        hub.metric("rest_step_rate_limited_total")
        logger.warning("rest_step_rate_limited worker_id=%s hijack_id=%s client=%s", worker_id, hijack_id, _client_id)
        await hub.emit_telemetry(
            "rate_limit.triggered",
            worker_id=worker_id,
...
async def _hijack_release(
    hub: TermHub,
    http_request: Request,
    worker_id: str,
    hijack_id: str,
) -> Any:
    """Release a hijack lease; the body of ``POST /worker/{id}/hijack/{hid}/release``."""
    hs = await hub.get_rest_session(worker_id, hijack_id)
    if hs is None:
        return JSONResponse({"error": "Invalid or expired hijack session."}, status_code=404)
...
def register_rest_routes(hub: TermHub, router: APIRouter) -> None:
    ...
    async def hijack_acquire(
        ...
        return await _hijack_acquire(hub, http_request, worker_id, request)
    ...
    async def hijack_heartbeat(
        ...
        return await _hijack_heartbeat(hub, worker_id, hijack_id, request)
    ...
    async def hijack_snapshot(
        ...
        return await _hijack_snapshot(hub, worker_id, hijack_id, wait_ms)
    ...
    async def hijack_events(
        ...
        return await _hijack_events(hub, worker_id, hijack_id, after_seq, limit)
    ...
    async def hijack_send(
        ...
        return await _hijack_send(hub, http_request, worker_id, hijack_id, request)
    ...
    async def hijack_step(
        ...
        return await _hijack_step(hub, http_request, worker_id, hijack_id)
    ...
    async def hijack_release(
        ...
        return await _hijack_release(hub, http_request, worker_id, hijack_id)
