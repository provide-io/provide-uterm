...
:class:`provide.uterm.server.bridge.hub.core.TermHub` (forwards to
...
* ``_REST_CLIENT_CACHE_MAX`` / ``_REST_CLIENT_EVICT_COUNT`` —
  ...
  :mod:`provide.uterm.server.bridge.hub.limiter` so existing
  ``from provide.uterm.server.bridge.hub.connections import ...`` imports keep
  working.
