...
export class DeckMuxGhostOverlay {
    ...
    showUser(userId, color, cols, rows) {
        if (!this._visible || cols === 0 || rows === 0)
            return;
    ...
    flashUser(userId, color, cols, rows) {
        if (!this._visible || cols === 0 || rows === 0)
            return;
    ...
    _positionBox(el, userCols, userRows) {
        if (this._ownCols === 0 || this._ownRows === 0)
            return;
