...
const _state = { injected: false };
export function _resetHijackCssState() {
    _state.injected = false;
}
export function injectHijackCss(baseUrl) {
    if (_state.injected)
        return;
...
export function escapeHijackHtml(value) {
    const el = document.createElement("span");
    el.textContent = String(value);
    return el.innerHTML;
}
export function buildHijackToolbarHtml(opts) {
    const p = (id) => `h-${opts.uid}-${id}`;
