...
const KEYSTROKE_AUTO_HIDE_MS = 2000;
export class DeckMuxControlPanel {
    ...
    showContextMenu(_userId, user, position, actions) {
        ...
        for (const action of actions) {
            ...
            if (action.danger)
                item.classList.add("dm-context-menu-item--danger");
    ...
    showRequestToast(fromName, fromColor, onAccept, onDeny) {
    ...
    showTransferToast(toName, toColor) {
        this._clearToastTimer();
    ...
    showAutoTransferWarning(toName, secondsRemaining) {
        this._clearToastTimer();
        this._stopCountdown();
    ...
    showAutoTransferComplete(toName) {
        this._stopCountdown();
        this.hideToasts();
    ...
    showKeystrokeQueue(userId, displayKeys, position) {
