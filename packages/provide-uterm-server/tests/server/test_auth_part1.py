...
# ---------------------------------------------------------------------------
# Finding #4: header-mode trust gating
# ---------------------------------------------------------------------------


class TestHeaderModeStartupGuard:
    """``auth.mode=header`` must refuse a non-loopback bind without trusted IPs."""

    def test_non_loopback_without_trusted_proxy_ips_rejected(self) -> None:
        ...
        cfg.auth.trusted_proxy_ips = []
        cfg.server.host = "0.0.0.0"
        with pytest.raises(ValueError, match="trusted_proxy_ips"):
            _validate_auth_config(cfg)

    def test_loopback_without_trusted_proxy_ips_ok(self) -> None:
        from provide.uterm.server import default_server_config
        from provide.uterm.server.app.auth import _validate_auth_config
        ...
        # Should not raise — loopback bind is safe (only local callers reach it).
        _validate_auth_config(cfg)

    def test_non_loopback_with_trusted_proxy_ips_ok(self) -> None:
        from provide.uterm.server import default_server_config
        from provide.uterm.server.app.auth import _validate_auth_config
        ...
        cfg.auth.header_mode_acknowledged = True
        cfg.auth.worker_bearer_token = "x" * 40
        cfg.auth.trusted_proxy_ips = ["10.0.0.5"]
        cfg.server.host = "0.0.0.0"
        _validate_auth_config(cfg)
