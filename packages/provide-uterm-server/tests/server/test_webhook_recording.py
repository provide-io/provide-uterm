...
async def test_webhook_recording_get_entries():
    ...
    async with respx.mock:
        ...
        route = respx.get(expected_url).mock(return_value=httpx.Response(200, json={"entries": mock_entries}))
        ...
        request = route.calls.last.request
        assert request.headers["Authorization"] == f"Bearer {secret}"
        # httpx.QueryParams are used, let's check them
...
async def test_webhook_recording_meta():
    ...
    async with respx.mock:
        ...
        route = respx.get(expected_url).mock(return_value=httpx.Response(200, json=mock_meta))
...
async def test_webhook_recording_get_entries_invalid_response():
    ...
    store = WebhookRecordingStore(url)
    async with respx.mock:
        respx.get(f"{url}/sid/entries").mock(return_value=httpx.Response(200, json=["not", "a", "dict"]))
...
async def test_webhook_recording_get_failure_exception():
    ...
    store = WebhookRecordingStore(url)
    async with respx.mock:
        respx.get(f"{url}/sid/meta").mock(side_effect=httpx.ConnectError("fail"))
