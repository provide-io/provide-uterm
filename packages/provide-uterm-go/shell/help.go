...
// helpText is the top-level "help" output. Byte-for-byte port of
// commands/help.py:_HELP.
var helpText = Heading("ushell commands") +
	FmtKVDefault("help [cmd]", "this help text, or detail for <cmd>") +
	FmtKVDefault("clear", "erase the terminal screen") +
	FmtKVDefault("py <expr>", "evaluate Python expression or statement") +
	FmtKVDefault("sessions", "list active sessions from KV registry") +
	FmtKVDefault("sessions kill <id>", "force-terminate a session DO") +
	FmtKVDefault("kv list", "list all KV session keys") +
	FmtKVDefault("kv get <key>", "read a KV value") +
	FmtKVDefault("kv set <key> <value>", "write a KV entry") +
	FmtKVDefault("kv delete <key>", "delete a KV entry") +
	FmtKVDefault("fetch [-X METHOD] <url> [body]", "HTTP request (GET by default)") +
	FmtKVDefault("render [flags] <url>", "render image as ANSI art") +
	FmtKVDefault("cast [--fps N] [--loop] <url>", "fetch and replay an asciicast v2 file") +
	FmtKVDefault("storage list", "list DO storage keys") +
	FmtKVDefault("storage get <key>", "read a DO storage value") +
	FmtKVDefault("env", "show available context keys") +
	FmtKVDefault("exit / quit", "end this shell session")

// commandHelp holds the per-command detail text for "help <cmd>". Byte-for-byte
// port of commands/help.py:_COMMAND_HELP.
