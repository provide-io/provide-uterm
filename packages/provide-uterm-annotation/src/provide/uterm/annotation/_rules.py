...
BUILTIN_RULES: list[DetectionRule] = [
    ...
    DetectionRule(
        ...
        severity="high",
        description_template="AWS access key detected in {event_type}",
        event_types=_BOTH,
    ...
    DetectionRule(
        ...
        label="credential_exposure",
        pattern=re.compile(r"gh[psourx]_[A-Za-z0-9_]{8}"),
        severity="high",
        description_template="GitHub token detected in {event_type}",
        event_types=_BOTH,
        category="credentials",
    ),
    DetectionRule(
        ...
        label="credential_exposure",
        pattern=re.compile(r"(?i)(password|secret|token|api_key)\s*[=:]"),
        severity="high",
        description_template="Secret assignment detected in {event_type}",
        event_types=_BOTH,
        category="credentials",
    ),
    DetectionRule(
        ...
        label="credential_exposure",
        pattern=re.compile(r"Bearer\s+\S{8}"),
        severity="high",
        description_template="Bearer token detected in {event_type}",
        event_types=_BOTH,
        category="credentials",
    ),
    DetectionRule(
    ...
    DetectionRule(
        ...
        pattern=re.compile(r"\brm\s+(-[rRf]{2,}|-[rR]\s+-f|-f\s+-[rR])"),
        severity="critical",
    ...
    DetectionRule(
        rule_id="conn.curl",
        label="outbound_connection",
        pattern=re.compile(r"\bcurl\b.*https?://"),
        severity="info",
    ...
    DetectionRule(
        rule_id="conn.wget",
        label="outbound_connection",
        pattern=re.compile(r"\bwget\b.*https?://"),
        severity="info",
