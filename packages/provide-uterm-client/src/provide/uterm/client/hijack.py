...
def _sanitize(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        return {
            k: (
                "***"
                if any(s in k.lower() for s in ("token", "secret", "password", "key", "auth", "session_id"))
                else _sanitize(v)
            )
...
class HijackClient:
    """Async REST client for the provide-uterm hijack + session API.

    Parameters
    ----------
    base_url:
        Root URL of the provide-uterm server (e.g. ``http://localhost:8780``).
    entity_prefix:
        Path prefix for worker endpoints.  ``"/worker"`` for provide-uterm,
