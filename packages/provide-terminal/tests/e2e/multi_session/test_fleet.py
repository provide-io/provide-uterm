...
from ..conftest import _drain_all, _drain_until, _ws_url, _snapshot_msg
...
async def test_cross_session_eventbus_isolation_under_load() -> None:
    """3 sessions × 10 concurrent snapshots — each subscriber gets exactly its own events."""
    async with live_server_with_bus(THREE_SESSIONS, label="fleet_isolation") as (hub, base_url):
        async with (
...
async def test_fleet_broadcast_storm_with_dead_socket_cleanup() -> None:
    ...
    async with live_server_with_bus(bs_sessions, label="broadcast_storm") as (hub, base_url):
        # Connect 3 workers
        async with (
