...
Wires PamNotifyListener (provide-terminal-pty) into the session registry so that
sshd logins tracked by pam_uterm.so automatically become provide-terminal sessions.
...
Two modes, configured via ``ServerConfig.pam.mode``:
  ...
  capture
    ...
    IS the live SSH session, observable via the provide-terminal UI.
...
if TYPE_CHECKING:
    from provide.terminal.pty.pam_listener import PamEvent  # type: ignore[import-untyped]
...
async def run_pam_integration(config: object, registry: object) -> None:
    ...
    try:
        ...
        logger.warning("pam_integration: provide-terminal-pty not installed — PAM listener disabled")
        return
...
async def _on_open(
    ...
    from provide.terminal.server.models import PamConfig  # noqa: TC001 — runtime cast needed
