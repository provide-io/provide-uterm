...
#
"""DeckMux integration for the :mod:`provide.terminal.auth` identity frame.

The SSH gateway (:mod:`provide.terminal.gateway`) emits an ``identity``
control frame as the first WebSocket message on connections where a
:class:`~provide.terminal.auth.SSHKeyResolver` accepted the client's
...
- :func:`parse_identity_frame` turns a raw control-channel dict into a
  :class:`~provide.terminal.auth.ResolvedIdentity`, returning ``None``
...
- :func:`identity_as_principal` adapts a ``ResolvedIdentity`` to the
  ...
  :class:`~provide.terminal.deckmux._hub_mixin.DeckMuxMixin` already
...
from provide.terminal.deckmux._names import generate_color, generate_initials, generate_name
from provide.terminal.deckmux._presence import UserPresence
