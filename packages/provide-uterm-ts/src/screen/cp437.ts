 ...
 * software; it maps bytes 0x80-0xFF to box-drawing and accented characters.
 * Node has no built-in codec for it, so the table below was exported from the
 * CPython codec by `testdata/gen_screen_golden.py` rather than transcribed by
 * hand, and the corpus asserts it entry by entry.
 */

/** Byte value to the code point it decodes to, for all 256 bytes. */
export const CP437_TABLE: readonly number[] = [
...
/** Code point to the byte that encodes it, built from {@link CP437_TABLE}. */
const CP437_REVERSE = new Map<number, number>(CP437_TABLE.map((codePoint, byte) => [codePoint, byte]));

/** The byte substituted for a character CP437 cannot represent. */
const REPLACEMENT_BYTE = 0x3f;

/** Decode bytes from CP437 to a string. */
export function decodeCp437(data: Uint8Array): string {
  let out = "";
  for (const byte of data) {
    out += String.fromCodePoint(CP437_TABLE[byte] as number);
  }
  return out;
}
 ...
 *
 * Characters with no CP437 representation become `?`, matching CPython's
 * `encode("cp437", errors="replace")`.
 */
export function encodeCp437(text: string): Uint8Array {
  const out: number[] = [];
  for (const char of text) {
    out.push(CP437_REVERSE.get(char.codePointAt(0) as number) ?? REPLACEMENT_BYTE);
  }
