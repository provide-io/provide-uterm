...
export async function fetchProfile(profileId) {
    try {
        return await apiJson(`/api/profiles/${encodeURIComponent(profileId)}`);
    }
    catch {
        return null;
    }
...
export async function createProfile(payload) {
    return apiJson("/api/profiles", "POST", payload);
}
export async function deleteProfile(profileId) {
    await apiJson(`/api/profiles/${encodeURIComponent(profileId)}`, "DELETE");
}
export async function connectFromProfile(profileId, password) {
    return apiJson(`/api/profiles/${encodeURIComponent(profileId)}/connect`, "POST", password ? { password } : {});
}
