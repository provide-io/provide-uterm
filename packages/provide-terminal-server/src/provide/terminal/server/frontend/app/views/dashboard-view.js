...
function profilesSectionMarkup(profiles, appPath) {
...
export async function renderDashboard(root, bootstrap) {
  ...
  `;
    ...
    async function loadAll(statusEl, contentEl) {
        try {
    ...
    content.addEventListener("click", (e) => {
        ...
        const deleteProfileBtn = target.closest(".btn-delete-profile");
        if (deleteProfileBtn) {
            const pid = deleteProfileBtn.dataset.profileId;
            if (!pid)
                return;
            if (!window.confirm(`Delete profile? This cannot be undone.`))
                return;
