...
function decodeControlFrames(raw) {
    const frames = [];
    let pos = 0;
...
export async function renderInspect(root, bootstrap) {
    if (!bootstrap.session_id)
        throw new Error("inspect bootstrap missing session_id");
  ...
  `;
    const listEl = requireElement("#inspect-list", root);
    const detailEl = requireElement("#inspect-detail", root);
    const countEl = requireElement("#inspect-count", root);
    const statusEl = requireElement("#inspect-status", root);
    const methodFilter = requireElement("#inspect-method-filter", root);
    const urlFilter = requireElement("#inspect-url-filter", root);
    const state = {
        exchanges: [],
        selected: null,
        ws: null,
    ...
    function updateList() {
        ...
        const filtered = state.exchanges.filter((ex) => {
            if (mf && ex.request.method !== mf)
                return false;
            if (uf && !ex.request.url.toLowerCase().includes(uf))
                return false;
            return true;
        });
    ...
    listEl.addEventListener("click", (e) => {
        const row = e.target.closest(".inspect-row");
        if (row)
    ...
    // Toggle buttons
    const inspectToggle = requireElement("#inspect-inspect-toggle", root);
    const interceptToggle = requireElement("#inspect-intercept-toggle", root);
    function sendWsJson(msg) {
        if (state.ws && state.ws.readyState === WebSocket.OPEN) {
            state.ws.send(JSON.stringify(msg));
        }
    }
    function sendAction(id, action, headers, bodyB64) {
        const msg = { type: "http_action", id, action };
        if (headers)
            msg.headers = headers;
        if (bodyB64)
            msg.body_b64 = bodyB64;
        sendWsJson(msg);
        const ex = state.exchanges.find((e) => e.id === id);
        if (ex) {
    ...
    function showModifyEditor(id) {
      ...
      <div class="inspect-editor" id="inspect-editor">
        ...
        requireElement("#editor-send", root).addEventListener("click", () => {
            const newBody = requireElement("#editor-body", root).value;
            const newHeaders = {};
            detailEl.querySelectorAll(".hdr-key").forEach((el) => {
                const keyEl = el;
                const valEl = detailEl.querySelector(`.hdr-val[data-idx="${keyEl.dataset.idx}"]`);
                if (keyEl.value && valEl)
                    newHeaders[keyEl.value] = valEl.value;
            });
    ...
    inspectToggle.addEventListener("click", () => {
        ...
        sendWsJson({ type: "http_inspect_toggle", enabled: state.inspectEnabled });
    ...
    interceptToggle.addEventListener("click", () => {
        ...
        sendWsJson({ type: "http_intercept_toggle", enabled: state.interceptEnabled });
    });
    // Connect WebSocket
