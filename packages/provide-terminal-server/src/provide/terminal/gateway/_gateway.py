...
from provide.terminal.gateway._iac_negotiate import IacNegotiator
    ...
    *,
    ...
    for event in events:
        if isinstance(event, DataChunk):
            return False
        handled = await _handle_ws_control_frame(
            event.control, token_holder, write_fn, token_file=token_file
        ) or handled
    return handled
            ...
            for event in events:
                if isinstance(event, ControlChunk):
                    await _handle_ws_control_frame(
                        event.control, token_holder, _write_fn, token_file=token_file
                    )
                    continue
    ...
    *,
    ...
    async with websockets.connect(ws_url) as ws:
        ...
        t1 = asyncio.create_task(
            _tcp_to_ws(reader, ws, telnet=telnet, negotiator=negotiator, writer=writer)
        )
