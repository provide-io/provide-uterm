...
import pytest
from provide.terminal.bridge.hub import TermHub, PolicyContext
from provide.terminal.bridge.hub.ext import RedactionRule, OutputPolicyGate
...
async def test_hub_output_redaction_broadcast() -> None:
    ...
    for call in ws.send_text.call_args_list:
        ...
        for event in events:
            if event.kind == "data":
                if "[STRIPE_SECRET]" in event.data:
                    found_redacted = True
