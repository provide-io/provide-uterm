...
from provide.terminal.bridge.hub.ext import PolicyDecision, PolicyContext, FanOutPolicyGate
...
async def test_fanout_passthrough_allow(controller, hub, gate):
    ...
    async def fake_send_parallel(group, data, q_ms, m_ms):
        nonlocal called
        called = True
        return FanOutResult(group_id=group.group_id, send_id="s1", command=data, sent_at=time.time(), results=[], divergent_sessions=[], failed_sessions=[])
...
async def test_fanout_approval_rbac_admin_only(controller, hub, gate):
    ...
    async def is_admin_false(p): return False
    ...
    async def is_admin_true(p): return True
