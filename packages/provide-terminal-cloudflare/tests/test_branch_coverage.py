...
async def test_wait_for_prompt_snapshot_none_during_poll() -> None:
    ...
    from provide_terminal_cloudflare.api.http_routes._shared import _wait_for_prompt
...
async def test_alarm_worker_connected_no_set_alarm() -> None:
    ...
    mock_kv = AsyncMock()
    with patch("provide_terminal_cloudflare.do._session_runtime_io.update_kv_session", mock_kv):
        await rt.alarm()
