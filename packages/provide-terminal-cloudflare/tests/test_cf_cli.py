...
import pytest
from provide_terminal_cloudflare.cli import build_parser, cmd_build, cmd_dev, main
...
class TestCmdBuild:
    ...
    def test_build_fails_when_entry_missing(self, tmp_path: Path) -> None:
        ...
        with patch("provide_terminal_cloudflare.cli._package_root", return_value=tmp_path):
            args = SimpleNamespace()
            result = cmd_build(args)
        assert result == 1
...
class TestCmdDevDeploy:
    def test_cmd_dev_calls_pywrangler(self) -> None:
        ...
        with (
            patch("provide_terminal_cloudflare.cli._require_pywrangler"),
            patch("provide_terminal_cloudflare.cli._run", return_value=0) as mock_run,
        ):
            result = cmd_dev(args)
        assert result == 0
    ...
    def test_cmd_deploy_calls_pywrangler(self) -> None:
        from provide_terminal_cloudflare.cli import cmd_deploy
    ...
    def test_require_pywrangler_raises_when_not_found(self) -> None:
        from provide_terminal_cloudflare.cli import _require_pywrangler
...
class TestMain:
    def test_main_build_subcommand(self) -> None:
        with patch("provide_terminal_cloudflare.cli.cmd_build", return_value=0) as mock:
            result = main(["build"])
        assert result == 0
        mock.assert_called_once()

    def test_main_runtime_error_returns_1(self) -> None:
        with patch("provide_terminal_cloudflare.cli.cmd_build", side_effect=RuntimeError("oops")):
            result = main(["build"])
        assert result == 1
    ...
    def test_run_helper_invokes_subprocess(self) -> None:
        from provide_terminal_cloudflare.cli import _run
    ...
    def test_main_module_import(self) -> None:
        ...
        mod = importlib.import_module("provide_terminal_cloudflare.__main__")
...
class TestServeAssetFallbacks:
    def test_path_file_fallback_serves_existing_file(self, tmp_path: Path) -> None:
        ...
        from provide_terminal_cloudflare.ui import assets as assets_mod
