...
_INCIDENT_DIR = Path("/tmp/incident")  # noqa: S108
...
def _write_incident_data() -> None:
    ...
    for i in range(150):
        ...
        if i in {42, 67, 89, 91, 93, 95, 98, 100, 102, 105, 110, 115, 120, 125, 130}:
            ...
            syslog_lines.append(f"{ts} api-server INFO: request completed status=200 path=/api/v1/health latency={12 + i % 30}ms")
    ...
    # Docker ps output
    (_INCIDENT_DIR / "docker_ps.txt").write_text(textwrap.dedent("""\
        NAMES            STATUS          PORTS
    ...
    (_INCIDENT_DIR / "docker_logs_recovery.txt").write_text(textwrap.dedent("""\
...
def record(base_out: Path = BASE_OUT) -> dict[str, Path | None]:
    ...
    print(f"  Recording {len(all_steps)} perspectives ({sum(len(s) for s in all_steps.values())} total steps)...", flush=True)
