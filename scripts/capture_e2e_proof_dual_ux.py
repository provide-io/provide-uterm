...
"""
    ...
    fleet_proc = subprocess.Popen(["python3", "mock_fleet.py"], 
...
"""
    ...
    server_proc = subprocess.Popen(["uv", "run", "uterm-server", "--config", "proof-ux.toml", "--port", "8000"],
                                  stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
    time.sleep(5)
    ...
    try:
        ...
        if os.path.exists("mock_fleet.py"): os.remove("mock_fleet.py")
        if os.path.exists("proof-ux.toml"): os.remove("proof-ux.toml")
        print("\n--- PROOF COMPLETE ---\n")
