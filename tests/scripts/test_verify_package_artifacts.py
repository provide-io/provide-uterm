...
_SCRIPT_PATH = Path(__file__).resolve().parents[2] / "scripts" / "verify_package_artifacts.py"
...
def test_required_package_data_includes_py_typed_and_server_frontend() -> None:
    by_name = {pkg.name: pkg.required_members for pkg in artifacts.PUBLISHED_PACKAGES}
